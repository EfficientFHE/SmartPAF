...
def CT_train(sign_type, degree, sign_scale, scale_path, layer_name_input, sign_nest_dict, batch_size, input_data_dirctory, output_floder_suffix, epoch = 0):
    print(sign_type)
    key = layer_name_input
    ...
    input_data = torch.load(input_data_dirctory +file_name)
    ...
    print("name: ", key)  
    my_model = SiLU_minmax_layer(sigmoid=sign_module)
    ref_model = nn.SiLU()
...
def CT_collection(model, sign_nest_dict, layer_name, sign_type, sign_scale, scale_path,  dataset, data_path, degree = 7, output_floder_suffix = "dynamic"):
    ...
    CT_train(sign_type = sign_type,degree = degree ,sign_scale = sign_scale, scale_path = scale_path, layer_name_input = layer_name,
             sign_nest_dict = sign_nest_dict, batch_size = batch_size, input_data_dirctory = data_path, output_floder_suffix = output_floder_suffix)

def train_network(input_data_dirctory, model, sign_type, valid_data_loader, train_data_loader, start_layer_name, max_layer_counter, lr_c, lr_w, dataset, degree = 7, CT_suffix = "dynamic"):
    sign_type = sign_type
    input_data_dirctory = input_data_dirctory
    group_epochs = global_config["PA_AT"]["group_epochs"]

    folder_name = "CT_" + sign_type + "_S" + CT_suffix+"_40s/"
    ...
    for key in sign_nest_dict:
        ...
        CT_collection(model = copy.deepcopy(G_model), sign_nest_dict = sign_nest_dict, layer_name = key, sign_type = sign_type,
                      sign_scale = 0, scale_path = None,  dataset = dataset, data_path = input_data_dirctory, degree= degree, output_floder_suffix=CT_suffix)
        
        file_name = key + "_coef.pt"
        scale_name = key + "_scale.pt"
        sign_dict = sign_nest_dict[key]
        sign_scale = 0
        coef = torch.load(input_data_dirctory + folder_name + file_name)
        degree = len(coef.tolist()[0])
        sign_module_CT = Sigmoid_minmax_layer(coef=coef, degree=[degree],scale=1)
        print(file_name)
        my_layer_CT = SiLU_minmax_layer(sigmoid=sign_module_CT)
...
if __name__ == "__main__":
    ...
    train_network(model=model, sign_type = args.sign_type, start_layer_name = start_layer_name,
                  ...
                  input_data_dirctory = sub_working_directory, dataset = args.dataset, degree=7, CT_suffix="polyfit_o7")
